 ...
 */
$GLOBALS['TL_DCA']['tl_calendar_events'] =
[
	...
	[
		...
		[
			...
			'child_record_callback'   => ['tl_calendar_events', 'listEvents'],
		...
		[
			...
			[
				...
				'button_callback'     => ['tl_calendar_events', 'toggleIcon']
			],
			'show' =>
			[
	...
	'palettes' =>
	[
		'__selector__'                => ['addTime', 'addImage', 'recurring', 'addEnclosure', 'source', 'published'],
	...
	[
		...
		[
			...
			'save_callback' =>
			[
				['tl_calendar_events', 'generateAlias']
			],
		...
		[
			...
			'save_callback' =>
			[
				['tl_calendar_events', 'setEmptyEndDate']
			],
			'sql'                     => "int(10) unsigned NULL"
		],
		'location' =>
		...
		[
			...
			'filter'                  => true,
			'inputType'               => 'radio',
			'options_callback'        => ['tl_calendar_events', 'getSourceOptions'],
			'reference'               => &$GLOBALS['TL_LANG']['tl_calendar_events'],
		...
		[
			...
			'exclude'                 => true,
			'inputType'               => 'select',
			'options_callback'        => ['tl_calendar_events', 'getArticleAlias'],
...
class tl_calendar_events extends Backend
{
	...
	{
		return ' <a href="contao/page.php?do='.Input::get('do').'&amp;table='.$dc->table.'&amp;field='.$dc->field.'&amp;value='.str_replace(['{{link_url::', '}}'], '', $dc->value).'" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['pagepicker']).'" onclick="Backend.getScrollOffset();Backend.openModalSelector({\'width\':768,\'title\':\''.specialchars(str_replace("'", "\\'", $GLOBALS['TL_LANG']['MOD']['page'][0])).'\',\'url\':this.href,\'id\':\''.$dc->field.'\',\'tag\':\'ctrl_'.$dc->field . ((Input::get('act') == 'editAll') ? '_' . $dc->id : '').'\',\'self\':this});return false">' . Image::getHtml('pickpage.gif', $GLOBALS['TL_LANG']['MSC']['pagepicker'], 'style="vertical-align:top;cursor:pointer"') . '</a>';
	}
